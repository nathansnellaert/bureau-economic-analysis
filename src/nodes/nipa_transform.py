...
def transform_table_frequency(
    ...
    Returns `(table, column_line_descriptions)` — the second element maps each
    slugified column name to the original `LineDescription` from the raw data
    so the caller can publish semantic column descriptions.
    """
    if not records:
        return None
    ...
    return pa.Table.from_pylist(rows, schema=schema), col_to_desc
...
def run():
    ...
    for table_info in catalog:
        ...
        for frequency in ['annual', 'quarterly', 'monthly']:
            ...
            if max_date[:4] < cutoff_year:
                ...
                stale_id = make_dataset_id(table_name, description, frequency, suffix)
    ...
    print(
        f"\nComplete: {datasets_created} datasets created, "
        f"{datasets_unchanged} unchanged, "
        f"{datasets_skipped} tables skipped (no raw data), "
